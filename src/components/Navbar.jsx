      ...
      <nav className="bg-white dark:bg-gray-900 fixe w-full z-20 top- left-0 border-b border-gray-200 dark:border-gray-600">
        <div className="max-w-screen-xl flex flex-wrap items-center justify-between relative mx-auto p-4">
          ...
          <div className='flex gap-3 order-2'>
            <button onClick={() => setMode(!mode)} className='dark:text-white'>{mode ? <BsFillSunFill /> : <BsMoonFill/>}</button>
            <button className='relative dark:text-white' onClick={() => navigate('/cart')}>
              <span className='absolute   bg-blue-500 text-white px-2 text-xs rounded-lg'>{savat.length}</span>
